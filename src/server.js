...
module.exports = async (client) => {
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://anka.qtr1.repl.co/user/${a.id}) The user named ** site ** tried to log in, but could not log in because she was blocked from the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
                    message: "You have been blocked from Anka.",
                    logout: true
                })
                req.logout();
            });
            ...
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://anka.qtr1.repl.co/user/${a.id}) User named **site** logged in.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/api/status", checkMaintence, async (req, res) => {
        const botdata = await botsdata.find();
        renderTemplate(res, req, "botlist/status.ejs", {
            config,
            roles,
            botdata,
            getuser
    ...
    app.get("/index/light", checkMaintence, async (req, res) => {
        const botdata = await botsdata.find();
        res.redirect(`https://anka.qtr1.repl.co/bots?error=true&message=We%20Didn%27t%20Make%20This%20Page `)
    });
    app.get("/dc", (req, res) => {
        res.redirect('');
    });
    ...
    app.post("/bot/:botID/vote", checkMaintence, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.votes).send(new Discord.MessageEmbed().setDescription(`<@${req.user.id}> Thanks For Supporting <@${req.params.botID}>`).setColor(`#ff0000`) );
    ...
    const checkAdmin = async (req, res, next) => {
        if (req.isAuthenticated()) {
            if (client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.yonetici) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.moderator) || client.guilds.cache.get(config.server.id).members.cache.get(req.user.id).roles.cache.get(roles.workers)) {
                next();
            } else {
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<a:maintence:833375738547535913> Anka has been switched to __maintance__ due to **${req.body.reason}**`).then(a => {
            new maintenceSchema({
                server: config.server.id,
    ...
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("anka", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`<a:online:833375738785824788> Anka are **active** again!\n[Click to redirect website](https://anka.qtr1.repl.co)`)
            .setFooter("Anka © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~ <a:maintence:833375738547535913> Anka has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesTopic)
        })
    ...
    });et("/api/bots/:botID", async (req, res) => {
    ...
    //------------------- API -------------------//    //------------------- API -------------------//
    app.use((req, res) => {
        res.status(404).redirect("/error?code=404&message=Did%20You%20Lost%20Your%20Way.")
    });
};
