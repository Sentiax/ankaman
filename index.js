...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`(!) Bot ${files.length} Installed as a command.`);
    files.forEach(f => {
...
client.on('ready',async () => {
    console.log("(!) Bot "+client.user.tag+" successfully connected to discord.");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: ''+bots.length+' Bots' }, status: "idle" });
    client.channels.cache.get(`937021975778459749`).send(new Discord.MessageEmbed().setDescription(`<a:UP:938281697726857236> **API** : ***UP***

    <a:UP:938281697726857236> **Mongoose :** ***UP***

    <a:UP:938281697726857236> **FireBase :** ***UP***`).setColor(`#ff0000`).setFooter(`Due API : https://anka.qtr1.repl.co/api/status` , `${client.user.displayAvatarURL()}`).setAuthor("DueAPI",  `${client.user.displayAvatarURL()}` , "https://anka.qtr1.repl.co/api/status") );
});
/*=======================================================================================*/
